"""
MNIST Adam Experiment — forward_xl evaluation
...
from src.training import train_step_optax
...
def main():
    print("=" * 60)
    print("  PCN MNIST Adam — forward_xl (single-pass top-layer)")
    print("=" * 60)
    ...
    BATCH_SIZE      = 512
    ETA_X           = 0.1
    ETA_W           = 0.001  # Smaller learning rate for Adam
    INFERENCE_STEPS = 50
    SEED            = 42
    EVAL_EVERY      = 5
    ...
    optimizer = optax.adam(learning_rate=ETA_W)
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, yb, _ in make_batches(x_train, y_train_oh, y_train, BATCH_SIZE, subkey):
            params, opt_state, metrics = train_step_optax(
                params, opt_state, xb, yb, optimizer,
                eta_x=ETA_X,
    ...
    if final_test < 0.50:
        ...
        print("\n  [PASS] Reasonable MNIST accuracy — forward_xl + Adam is working!")
